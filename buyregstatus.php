...
include 'PHP/loginuser.php';
	$website_prem_query = "SELECT * FROM  buzzypaidservices WHERE buzzypaidservice_id=7";	
	foreach ($connread->query($website_prem_query) as $row) {
	...
	$buzzypaidservice_price=$row['buzzypaidservice_price'];	
	 ...
	 if($session_buzzyuser_credits>=$buzzypaidservice_price){	  
        $update_userstatus_query = "UPDATE buzzyusers SET buzzyuser_status=0,buzzyuser_status_timestamp=:buzzyuser_status_timestamp WHERE buzzyuser_id=$session_user_id";
