...
if($session_buzzyuser_image!='profile-icon1.jpg' AND $session_buzzyuser_genre!=10 AND $session_buzzyuser_age!='0' AND $session_buzzyuser_first_name!='' AND $session_buzzyuser_last_name!='' AND $session_buzzyuser_location==''){
     header('Location: '.$link_prefix.'index.php');		
}	
else if($session_buzzyuser_location!=', ' AND $session_buzzyuser_image!='profile-icon1.jpg'){
header('Location:'.$link_prefix.'index.php');	
} 
?>
<!DOCTYPE html>
...
<?php
if(!isset($_GET["add-gender"]) AND !isset($_GET["add-so"]) AND !isset($_GET["add-main-image"]) AND !isset($_GET["full_name"]) AND !isset($_GET["location"])){
?>
...
$buzzyuser_first_name=$_POST['buzzyuser_first_name'];
$buzzyuser_last_name=$_POST['buzzyuser_last_name'];
$OK = false;
        $update_userbd_query = "UPDATE buzzyusers SET buzzyuser_first_name=:buzzyuser_first_name,buzzyuser_last_name=:buzzyuser_last_name WHERE buzzyuser_id=$session_user_id";
        $stmt = $connwrite->prepare($update_userbd_query);
        // bind the parameters and execute the statement
        $stmt->bindParam(':buzzyuser_first_name', $buzzyuser_first_name, PDO::PARAM_STR);	
        $stmt->bindParam(':buzzyuser_last_name', $buzzyuser_last_name, PDO::PARAM_STR);	
        // execute and get number of affected rows
        $stmt->execute();		
        $OK = $stmt->rowCount();	
...
<h3><?php echo $last_name;?></h3>
 <input type="text" name="buzzyuser_last_name" placeholder="<?php echo $last_name;?>" required class="form-control"
                               id="recipient-name">
<br>
							   <button style="width:100%;" type="submit" name="update_fname" class="buttontr btn btn-primary btn-lg">Update Full name</button>							   
</form>
</div>
<?php } ?>
...
$OK = false;
        $update_userbd_query = "UPDATE buzzyusers SET buzzyuser_location=:buzzyuser_location,buzzyuser_lat=:buzzyuser_lat,buzzyuser_long=:buzzyuser_long WHERE buzzyuser_id=$session_user_id";
        $stmt = $connwrite->prepare($update_userbd_query);
        // bind the parameters and execute the statement
...
<h3><?php echo $location;?></h3>
 <input type="text"  onFocus="geolocate()"  required id="autocomplete" name="buzzyuser_location" placeholder="<?php echo $location;?>" required class="form-control"
                               id="recipient-name">
<br>
							   <button style="width:100%;" type="submit" name="update_floc" class="buttontr btn btn-primary btn-lg">Update location</button>							   
</form>
</div>
<script>
