...
$OK = false;
$delete_friend_query = "DELETE FROM buzzyfriends  WHERE ((buzzyuser_id=$requester AND buzzyfriend=$session_user_id) OR (buzzyuser_id=$session_user_id AND buzzyfriend=$requester)) AND buzzyfriend_status=1";
$stmt = $connwrite->prepare($delete_friend_query);
$stmt->execute();
$OK = $stmt->rowCount();
		
		
header('Location: '.$link_prefix.'page.php?user-id='.$requester.'');		