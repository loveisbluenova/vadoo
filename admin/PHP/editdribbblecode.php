<?php
  $this_dribbble_query = "SELECT buzzynews_id,buzzynews_title,buzzynews_image,buzzynews_text,buzzynews_url,buzzynews_title_tag,buzzynews_metatag,
  ...
  buzzynews_comments,buzzynews_visitors,buzzynews_source_name,buzzynews_source_url FROM  buzzynews WHERE buzzynews_id=$dribbble_id";
  foreach ($connread->query($this_dribbble_query) as $row) {
  $taken_buzzynewscategory_id=$row['buzzynewscategory_id'];
  }
  
  if ($_SESSION["buzzyadmin_id"]==1){ 
  if (isset($_POST['update_dribbble'])) {
		...
		buzzynews_metatag=:buzzynews_metatag WHERE buzzynews_id=$dribbble_id";
