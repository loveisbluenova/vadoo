<?php
  $this_rss_query = "SELECT buzzynews_id,buzzynews_title,buzzynews_image,buzzynews_text,buzzynews_url,buzzynews_title_tag,buzzynews_metatag,
  ...
  buzzynews_comments,buzzynews_visitors,buzzynews_source_name,buzzynews_source_url FROM  buzzynews WHERE buzzynews_id=$rsslink_id";
  foreach ($connread->query($this_rss_query) as $row) {
  ...
  if (isset($_POST['update_rss'])) {
		...
		buzzynews_metatag=:buzzynews_metatag WHERE buzzynews_id=$rsslink_id";
        ...
        $stmt->execute();
        $OK = $stmt->rowCount();
		header('Location: rsslinks.php?add-settings-success=true');
		}
  }
  
   if ($_SESSION["buzzyadmin_id"]==2){
   if (isset($_POST['update_rss'])) {
	header('Location: rsslinks.php?onlydemo=true');
   }   
   }