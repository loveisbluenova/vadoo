...
$count_afake_users_query = "SELECT COUNT(*) FROM  buzzyfakeusers WHERE $total_filter_text buzzyfakeuser_age<=$buzzyfakefilter_agemax AND buzzyfakeuser_age>=$buzzyfakefilter_agemin AND buzzyfakeuser_insertedstatus=0";
...
$buzzyfakeuser_lat=$row['buzzyfakeuser_lat'];
$buzzyfakeuser_long=$row['buzzyfakeuser_long'];
	...
	$fa_buzzyuser_hash = $row['buzzyuser_hash'];
     $register_user_to_messages = "INSERT INTO  users
       (id,username,display_name,profile_image,type_status,last_seen,session_status)
		...
		$update_fakeuser_query = "UPDATE buzzyfakeusers SET buzzyfakeuser_insertedstatus=1 WHERE buzzyfakeuser_id=$buzzyfakeuser_id";
