...
header('Location:index.php?version-updated=true'); 	
	 ...
	 $create_fris = "CREATE TABLE IF NOT EXISTS buzzystripepayments(
    buzzystripepayment_id INT AUTO_INCREMENT PRIMARY KEY NOT NULL, 
	buzzyuser_id INT NOT NULL,
    buzzystripepayment_timestamp INT NOT NULL	
    )";	
         $stmt = $connwrite->prepare($create_fris);
        // bind the parameters and execute the statement			
        // execute and get number of affected rows
        $stmt->execute();
        $OK = $stmt->rowCount();		
	...
	$add_globone_query = "ALTER TABLE buzzyuserglobals ADD buzzy_stripekey VARCHAR(255) NOT NULL";
        // prepare the statement
        $stmt = $connwrite->prepare($add_globone_query);
        // bind the parameters and execute the statement
		// execute and get number of affected rows
        $stmt->execute();
        $OK = $stmt->rowCount();	
		
	$add_globtwo_query = "ALTER TABLE buzzyuserglobals ADD buzzy_stripeprice INT NOT NULL";
        // prepare the statement
        $stmt = $connwrite->prepare($add_globtwo_query);
        // bind the parameters and execute the statement
		// execute and get number of affected rows
        $stmt->execute();
        $OK = $stmt->rowCount();			
...
$update_website_soptions_query = "UPDATE buzzysiteoptions SET  buzzyupdatestatus=0,buzzyversion='1.1.2' WHERE buzzysiteoptions_id=1";
        $stmt = $connwrite->prepare($update_website_soptions_query);
        // bind the parameters and execute the statement			
        // execute and get number of affected rows
        $stmt->execute();
...
header('Location:index.php?version-updated=true'); 	
 ...
 else if($buzzyversion=='1.1.2' AND $buzzywebsite_status==0){
