...
<html lang="en" class="no-js">
 ...
 <div class="content">
  ...
  <tr>
    ...
    <tbody>
  <?php
  $all_pv_query = "SELECT  * FROM buzzyphoto_verimg"; 
  foreach ($connread->query($all_pv_query ) as $row) { 
  $buzzyphoto_verimg=$row['buzzyphoto_verimg'];
  $ppbuzzyuser_id=$row['buzzyuser_id'];
  $verph_users_query = "SELECT * FROM buzzyusers WHERE buzzyuser_id=$ppbuzzyuser_id AND buzzyphoto_ver=0"; 
  foreach ($connread->query($verph_users_query ) as $row) {
  ...
  <tr>
    ...
    <td><?php echo $buzzyuser_username;?></td>
	<td><img src="../img/<?php echo $buzzyphoto_verimg;?>"  class="img-thumbnail very-small-img" alt="<?php echo $buzzyuser_username;?>"/></a></td>
	<td><?php echo $buzzyuser_email;?></td>
	<td><a href="verify-photo.php?user-id=<?php echo $ppbuzzyuser_id;?>" style="color:#fff!important;" class="btn btn-success btn-sm" title="Approve photo">
	Approve photo
	</a></td>
	<td><a href="delete-vfphoto.php?user-id=<?php echo $ppbuzzyuser_id;?>" data-toggle="tooltip" data-placement="left" title="Delete photo"><span class="glyphicon glyphicon-trash"></span></a></td>
   </tr>
  <?php } ?>
  <?php } ?>  
   </tbody>
