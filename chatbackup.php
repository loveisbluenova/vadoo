...
border-color: transparent #c4e3fb;
display: block;
width: 0;
z-index: 1;
right: -6px;
...
<div id="wrapper">
    ...
    <div class="full-column">	
      <div class="content-wrap margin-reset" style="margin-top:0px!important; margin-bottom:0px; padding:0px;">
            <!-- messages -->
      <div class="left-msg" style="float:left;">
	  <div style="margin:0px!important; padding-top:20px!important; padding-bottom:20px!important;">
	 <?php
	 $all_users_msgquery = "SELECT * FROM friends WHERE user_id=$session_user_id OR friend=$session_user_id";
	 ...
	 $mss_users_query = "SELECT * FROM buzzyusers WHERE buzzyuser_id=$f_buzzyuser_id";
    ...
    $f_buzzyuser_username = $row['buzzyuser_username'];
	$f_buzzyuser_image = $row['buzzyuser_image'];
	$f_buzzyuser_onlinestatus=$row['buzzyuser_onlinestatus'];
	
	if($f_buzzyuser_onlinestatus==0){
	$on_span='<span style="color:#333; font-size:12px;">'.$offlinee.'</span>';	
	}
	else if($f_buzzyuser_onlinestatus==1){
	$on_span='<span style="color:#6fc848;  font-size:12px;">'.$onlinee.'</span>';	
	}	
...
if (strpos($f_buzzyuser_image,'facebook') !== false) {
	 ...
	 ?>
	 <div class="muser" id="mser<?php echo $f_buzzyuser_id;?>"  rel="muser<?php echo $f_buzzyuser_id;?>" style="cursor:pointer; padding-top:13px; padding-bottom:20px; margin-top:0px!important; margin-bottom:0px!importan;">
	<img src="<?php echo $final_image_prefix;?><?php echo $f_buzzyuser_image;?>" style="width:48px; 
	height:48px;  margin-right:10px!important; margin-left:20px; margin-top:0px!important;
	float:left!important;"> <span style="font-size:12px!important; font-weight:700;"><?php echo $f_buzzyuser_first_name;?></span> 
	<br>
	<?php echo $on_span;?>
	  </div>
	  ...
	  <?php } ?>
	  <script type="text/javascript">
$('#mser<?php echo $f_buzzyuser_id;?>').on('click', function(){
   var target = $(this).attr('rel');
    var target2 = $(this).attr('id');
    $('#mser<?php echo $f_buzzyuser_id;?>').addClass('activemmm').siblings("div").removeClass('activemmm');
    $('.send-msgbtn').attr( 'id', 'send-msgbtn<?php echo $f_buzzyuser_id;?>' ).siblings("div").removeClass('activemmm');	
   $("#"+target).show().siblings("div").hide();
   $("#head"+target).show().siblings("div").hide();
});
     </script>
	  <?php } ?>	  
	  </div>
	  </div>
	  <div style="float:left;" class="message-cont" >
...
if (strpos($f_buzzyuser_image,'facebook') !== false) {
	 ...
	 ?>	  
	  <div class="header-msg" id="headmser<?php echo $f_buzzyuser_id;?>" style="display:none; position:relative!important; background:#fff!important; border-bottom:1px solid #d8d8d8; width:100%;">
	  <div style="padding:20px;">
	  <h3><?php echo $f_buzzyuser_first_name;?> <?php echo $f_buzzyuser_last_name;?></h3>
	  </div>
	  </div>
	   <?php } ?>
...
$('#mser<?php echo $f_buzzyuser_id;?>').on('click', function(){
   ...
   $("#head"+target).show().siblings("div").hide();
	  ...
	  <div style="float:left;" class="message-contsmall" >
	   <?php
	 $all_users_msgquery = "SELECT * FROM friends WHERE user_id=$session_user_id OR friend=$session_user_id";
	 foreach ($connread->query($all_users_msgquery) as $row) { 
	 if($row['friend']==$session_user_id){
 ...
 $mss_users_query = "SELECT * FROM buzzyusers WHERE buzzyuser_id=$f_buzzyuser_id";
	...
	$f_buzzyuser_first_name = $row['buzzyuser_first_name'];	 
	 ...
	 ?>
	  ...
	  <?php
	       $all_msgquery = "SELECT * FROM messages WHERE (user_id=$session_user_id AND receiver=$f_buzzyuser_id) OR (user_id=$f_buzzyuser_id AND receiver=$session_user_id)";	 
...
if (strpos($msg_image,'facebook') !== false) {
	...
	$buzzyuser_chat_difference=time()-$row['time'];
     ...
     $message=$row['message'];	 
$patterns = array(':)',':(',':D');
$replacements = array('<img src="'.$link_prefix.'img/emo1fb.png" style="width:20px;"/>',
'<img src="'.$link_prefix.'img/emo3fb.png" style="width:20px;"/>',
'<img src="'.$link_prefix.'img/emo2fb.png" style="width:20px;"/>');

$message = str_replace($patterns, $replacements, $message);
	 ?>
	   <div style="margin-top:20px; margin-<?php echo $fll;?>:20px; margin-bottom:30px; float:<?php echo $fll;?>!important;"> 
                <img src="<?php echo $final_image_prefix;?><?php echo $msg_image;?>" class="media-object" alt="" style="float:<?php echo $fll;?>; margin-<?php echo $flr;?>:10px; width:48px; height:48px;" >
...
<div class="send-msgbtn" id="send-msgbtn<?php echo $f_buzzyuser_id;?>" style="position:absolute!important; background:#fff!important; width:100%;">
...
<input class="form-control" aria-label="Text input with multiple buttons"> <div class="input-group-btn"> 
<button type="button" class="btn btn-primary" style="border:1px solid #<?php echo $final_style_color;?>;" ><i class="fa fa-paper-plane" aria-hidden="true"></i> <?php echo $send;?></button> </div> </div>
</div>
</div>
