...
include 'PHP/loginuser.php';
	...
	$buzzypaidservice_price=$row['buzzypaidservice_price'];	
	 ...
	 if($session_buzzyuser_credits>=$buzzypaidservice_price){	  
        $add_blueemo_query = "INSERT INTO buzzyservicepayouts(buzzypaidservice_id,buzzyuser_id,buzzyservicepayout_timestamp)
		 	  VALUES(1,:buzzyuser_id,:buzzyservicepayout_timestamp)";
        // prepare the statement
        $stmt = $connwrite->prepare($add_blueemo_query);
        ...
        $stmt->execute();
        $OK = $stmt->rowCount();					
        header('Location: '.$link_prefix.'message.php?bougth-blueemo=true');			  				
        }		
		else if($session_buzzyuser_credits<$buzzypaidservice_price){	 
        header('Location: '.$link_prefix.'message.php?low-credit=true');				  
		}